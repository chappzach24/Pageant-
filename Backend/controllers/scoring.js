...
exports.getPageantScoringDetails = async (req, res) => {
  try {
    const pageant = await Pageant.findById(req.params.id).populate(
      "organization",
      "owner name"
    );
...
exports.updateParticipantScore = async (req, res) => {
  ...
  try {
    ...
    if (
      participant.pageant.status !== "in-progress" &&
      participant.pageant.status !== "completed"
    ) {
      return res.status(400).json({
        success: false,
        error:
          "Scores can only be updated for pageants in progress or completed",
      });
    }
...
exports.getPageantResults = async (req, res) => {
  try {
    ...
    pageant.ageGroups.forEach((ageGroup) => {
      const ageGroupParticipants = participants.filter(
      ...
      const participantsWithScores = ageGroupParticipants.map((participant) => {
        const totalScore = participant.categories.reduce(
          (sum, cat) => sum + (cat.score || 0),
          0
        );
        const avgScore =
          participant.categories.length > 0
            ? totalScore / participant.categories.length
            : 0;
...
exports.exportPageantResults = async (req, res) => {
  try {
    ...
    participants.forEach((participant) => {
      ...
      const row = {
        "Participant Name": `${participant.user.firstName} ${participant.user.lastName}`,
      ...
      participant.categories.forEach((cat) => {
        row[`${cat.category} Score`] = cat.score || 0;
        row[`${cat.category} Notes`] = cat.notes || "";
      });
